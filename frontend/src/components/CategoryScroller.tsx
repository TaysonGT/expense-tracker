...
export default function CategoryScroller() {
  ...
  return (
    <section className="mt-2">
      <h2 className="mb-3 text-sm font-medium text-primary">Categories</h2>
      <CategoryStrip
        isLoading={isLoading}
        dividerAfterManage
        className="-mx-4 ps-4"
      >
        {categories.map((c) => (
          <button
            ...
            className="shrink-0 rounded-full bg-card px-4 py-2 text-sm font-medium text-empty-title border border-border shadow-sm shadow-black/10 z-2 transition-transform active:scale-95"
          >
            {c.name}
          </button>
        ))}
