...
function VoiceCapture() {
  ...
  const pendingCount = useMemo(
    () => entities.filter((e) => !approvedIds.has(e.id)).length,
      ...
      <main className="max-w-md w-full px-4 grow flex flex-col justify-center">
        ...
        {phase === "recording" && (
          <div className="mt-10 flex flex-col items-center">
            <div className="min-h-[7rem] w-full rounded-2xl bg-white p-5 text-center shadow-sm ring-1 ring-gray-100">
              {transcript ? (
        ...
        {phase === "review" && (
          <div className="mt-6">
            ...
            >
              {pendingCount > 0 ? "Approve later" : "Done"}
            </button>
          </div>
        )}
      </main>
...
function RecordButton({
  ...
  return (
    <button
      ...
      className="relative flex h-24 w-24 items-center justify-center rounded-full text-white transition-transform active:scale-95 disabled:opacity-40"
      style={{
