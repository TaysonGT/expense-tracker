 ...
 * currency, member count, admin) and offers a Join button. On success the
 * backend has already activated the session, so we present the shared group-
 * switch overlay (skipActivate) and let the provider navigate to /home.
 * If they're already a member, we offer to switch to it via the overlay too.
 *
...
export default function GroupJoin() {
  ...
  const handleJoin = () => {
    ...
    setError(null);
    join.mutate(code, {
      onSuccess: (g) => switchToGroup({ id: g.id, name: g.name }, { skipActivate: true }),
